YOLO 底層吃的是 OpenCV，而 OpenCV 在這方面就是個「俗人」，它的守備範圍僅限於標準 8-bit 的 JPG 或 PNG。遇到帶有空間座標、16-bit 深度、甚至塞了十幾個光譜波段的 GeoTIFF 原檔，它根本不知道從何讀起，只能兩眼一抹黑當場罷工。為了解決這個硬體與套件的代溝，我們必須請出 Day 15 裝好的 GIS 神器 rasterio，把衛星圖降維成 OpenCV 看得懂的形狀。
